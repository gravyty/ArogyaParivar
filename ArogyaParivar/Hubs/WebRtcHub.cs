...
namespace ArogyaParivar.Hubs
{
    ...
    public class WebRtcHub : Hub
    {
        ...
        public void CallUser(string targetUserID,string arogyaID)
        {
            var callingUser = Users.SingleOrDefault(u => u.ConnectionId == Context.ConnectionId);
            var targetUser = Users.SingleOrDefault(u => u.UserID == targetUserID);
            
            // Make sure the person we are trying to call is still here
            if (targetUser == null)
            {
                // If not, let the caller know
                Clients.Caller.callDeclined(targetUserID, "The user you called has left.");
                return;
            }
            ...
            {
                Clients.Caller.callDeclined(targetUserID, string.Format("{0} is already in a call.", targetUser.Username));
                return;
            }

            // They are here, so tell them someone wants to talk
            Clients.Client(targetUserID).incomingCall(callingUser, arogyaID);
